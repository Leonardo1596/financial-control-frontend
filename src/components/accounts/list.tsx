...
export default function AccountsList({ accounts, onDelete, loading }: AccountsListProps) {
  ...
  return (
    <Table>
      <TableCaption className="pb-6">{!loading && accounts.length === 0 ? 'Nenhuma conta cadastrada.' : `Você possui ${accounts.length} contas ativas.`}</TableCaption>
      <TableHeader className="bg-slate-50/50">
        <TableRow className="hover:bg-transparent">
          <TableHead className="py-5 px-8 font-bold text-slate-600">Instituição</TableHead>
          <TableHead className="py-5 px-8 font-bold text-slate-600">Saldo Atual</TableHead>
          <TableHead className="text-right py-5 px-8 font-bold text-slate-600">Ações</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
            ...
            <TableCell className="py-5 px-8">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-slate-100 rounded-lg group-hover:bg-primary/10 transition-colors">
                  <Landmark className="h-4 w-4 text-slate-500 group-hover:text-primary" />
                </div>
